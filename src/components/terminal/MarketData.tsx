// components/terminal/MarketOverview.tsx - Vue d'ensemble du marché
...
const MarketOverview: React.FC<MarketOverviewProps> = ({ 
  ...
  // Données simulées pour une démonstration complète
  const mockMarketData: MarketData[] = [
    {
      ...
      liquidity: { usd: '500000000', base: '250000', quote: '500000000' },
  ...
  // Combiner les vraies données avec les données simulées
  const allMarketData = [
    ...Object.values(marketData),
    ...mockMarketData.filter(mock => !marketData[mock.address])
  ];

  // Filtrer selon les critères
  const filteredData = allMarketData.filter(data => {
    ...
    return parseFloat(b.marketCap) - parseFloat(a.marketCap); // Par défaut, trier par market cap
  });
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="flex space-x-1">
              {[
                ...
                { id: 'gainers', label: 'Gainers', count: marketStats.gainersCount },
                { id: 'losers', label: 'Losers', count: marketStats.losersCount }
              ].map(filterOption => (
                <button
                  key={filterOption.id}
